...
public class MediaMetadataRetriever {
  ...
  static {
    ...
    try {
        libFile = new File(Vitamio.getLibraryPath()+"libvitamiostlport_shared.so" );
    	if(libFile.exists()){
    		LIB_ROOT=Vitamio.getLibraryPath();
    	}else{
    		libFile = new File(Vitamio.getDataPath()+"libvitamiostlport_shared.so" );
    		if(libFile.exists()){
    ...
    if(LIB_ROOT==null){
    	...
    	loadFFmpeg_native("libffmpegvitamio.so");
    }else{
    	System.load(LIB_ROOT+ "libvitamiostlport_shared.so");
    	System.load(LIB_ROOT+ "libvscanner.so");
    	loadFFmpeg_native(LIB_ROOT+"libffmpegvitamio.so");
    }  

...
import com.seition.cloud.pro.newcloud.BuildConfig;
...
//    public static String sDomain = Api.APP_DOMAIN;
    ...
    public void applyOptions(Context context, GlobalConfigModule.Builder builder) {
        ...
        builder.baseurl(Service.DOMAIN_NAME)
...
//                if (!f.getActivity().getIntent().getBooleanExtra("isInitToolbar", false)) {
                if (f instanceof BaseBackFragment) {
                    //由于加强框架的兼容性,故将 setContentView 放到 onActivityCreated 之后,onActivityStarted 之前执行
                    //而 findViewById 必须在 Activity setContentView() 后才有效,所以将以下代码从之前的 onActivityCreated 中移动到 onActivityStarted 中执行
//                    f.getActivity().getIntent().putExtra("isInitToolbar", true);
                    //这里全局给Activity设置toolbar和title,你想象力有多丰富,这里就有多强大,以前放到BaseActivity的操作都可以放到这里
                    if (v.findViewById(R.id.toolbar) != null) {
                        if (f instanceof BaseBackFragment) {
//                            ((AppCompatActivity) f.getActivity()).setSupportActionBar((Toolbar) f.getActivity().findViewById(R.id.toolbar));
//                            ((AppCompatActivity) f.getActivity()).getSupportActionBar().setDisplayShowTitleEnabled(false);
                        } else {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//                                f.setActionBar((android.widget.Toolbar) v.findViewById(R.id.toolbar));
//                                activity.getActionBar().setDisplayShowTitleEnabled(false);
                            }
                        }
                    }
                    if (v.findViewById(R.id.toolbar_title) != null) {
                        if (f instanceof BaseBackFragment)
                            ((TextView) v.findViewById(R.id.toolbar_title)).setText(((BaseBackFragment) f).getTitle());
                    }
                    if (v.findViewById(R.id.toolbar_back) != null) {
                        if (f instanceof AreaMainFragment || f instanceof OrganizationApplyforFragment) {

...
import com.seition.cloud.pro.newcloud.home.mvp.ui.public_adapter.ArticleLibraryCategoryListAdapter;
...
public class ArticleLibraryCategoryPickPopupWindow extends BasePopWindow implements View.OnClickListener {
    ...
    private LibraryCategoryBean mSelectedProvince;
    private LibraryCategoryBean mSelectedCity;
    private LibraryCategoryBean mSelectedArea;

    private ArrayList<LibraryCategoryBean> firstDatas = new ArrayList<LibraryCategoryBean>();
    private ArrayList<LibraryCategoryBean> secondDatas = new ArrayList<LibraryCategoryBean>();
    private ArrayList<LibraryCategoryBean> thirdDatas = new ArrayList<LibraryCategoryBean>();

    private Map<String, LibraryCategoryBean> provinceMap = new HashMap<String, LibraryCategoryBean>();
    private Map<String, ArrayList<LibraryCategoryBean>> cityMap = new HashMap<String, ArrayList<LibraryCategoryBean>>();
    private Map<String, ArrayList<LibraryCategoryBean>> areaMap = new HashMap<String, ArrayList<LibraryCategoryBean>>();

    public ArticleLibraryCategoryPickPopupWindow(Activity activity, ArrayList<LibraryCategoryBean> firstData/*, Map<String, ArrayList<CommonCategory>> secondDatas, Map<String, ArrayList<CommonCategory>> areaMap*//*, String selected*/) {
        super();
        this.activity = activity;
        this.firstDatas = firstData;
        if (firstDatas!=null&&firstDatas.size()>0&&firstDatas.get(0) != null&&firstDatas.get(0).getChilds() != null)
        this.secondDatas = firstDatas.get(0).getChilds();
//        if (secondDatas!=null&&secondDatas.get(0) != null&&secondDatas.get(0).getChild() != null)
        if (secondDatas != null && secondDatas.size() > 0 && secondDatas.get(0).getChilds() != null)
            this.thirdDatas = secondDatas.get(0).getChilds();
        initPop();
    }
    LinearLayout empty;
...
//        popupWindow.setTouchable(true);
    ...
    ArticleLibraryCategoryListAdapter firstListAdapter;
    ArticleLibraryCategoryListAdapter seconListAdapter;
    ArticleLibraryCategoryListAdapter thirdListAdapter;

    private void initWheelView() {
        firstListAdapter = new ArticleLibraryCategoryListAdapter(activity, firstDatas, false, R.color.white);
        seconListAdapter = new ArticleLibraryCategoryListAdapter(activity, secondDatas, false, R.color.white);
        thirdListAdapter = new ArticleLibraryCategoryListAdapter(activity, thirdDatas, false, R.color.white);
        ...
        firstCategoryLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (firstDatas.get(firtListClickPosition).getChilds() == null) {
        ...
        secondCategoryLV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (secondDatas.get(seconListClickPosition).getChilds() == null) {
                    onDialogItemClickListener.onWindowItemClick(mSelectedProvince);
                    popupWindow.dismiss();
                }
...
//                setThirdListDataChange(0);
    ...
    private void setSeconListDataChange(int firtListClickPosition) {
        if (firstDatas.get(firtListClickPosition).getChilds() != null)
            secondDatas = firstDatas.get(firtListClickPosition).getChilds();
        else {
...
//            popupWindow.dismiss();
    ...
    private void setThirdListDataChange(int seconListClickPosition) {
        if (secondDatas.size() > 0 && secondDatas.get(seconListClickPosition).getChilds() != null)
            thirdDatas = secondDatas.get(seconListClickPosition).getChilds();
        else {

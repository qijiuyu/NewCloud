...
public class SettingPresenter extends BasePresenter<UserContract.Model, UserContract.SettingView> {
    ...
    @Inject
    public SettingPresenter(UserContract.Model model, UserContract.SettingView rootView) {
    ...
    public void getVersionInfo( ) {
        ...
        mModel.getVersionInfo()
                ...
                .subscribe(new ErrorHandleSubscriber<VersionInfo>(mErrorHandler) {
                    @Override
                    public void onNext(VersionInfo data) {
                        VersionInfo versionInfo = data.getData();
                        if(data.getCode() == 1){
                            mRootView.showVersionInfo(versionInfo);
                        }
                        else
                            mRootView.showMessage("版本获取失败");
                    }
                });
